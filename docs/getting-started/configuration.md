...
WireManager automatically saves scheduled backups into the `/app/backup` directory inside the container. To persist these encrypted archives on the host filesystem:
...
:::tip Retention & Persistence
Mounting `/app/backup` to an external host directory, dedicated volume, or NAS share guarantees that all archives generated by the automatic backup background worker survive container redeployments and can easily be replicated off-site.
:::
