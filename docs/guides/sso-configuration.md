...
3. The `FRONTEND_URL` environment variable properly configured in your `docker-compose.yaml` under `wiremanager-api`.
...
services:
  wiremanager-api:
    environment:
      - FRONTEND_URL=http://<server-ip>:3002 # or https://vpn.yourdomain.com
```
...
| **Valid Redirect URI** | `http://<api-host>:5070/signin-oidc`<br/>*(or `https://<api-domain>/signin-oidc`)* | The ASP.NET Core OpenID Connect handler endpoint. |

:::caution Redirect URI Points to the Backend API
Note that the OAuth2/OIDC redirect URI must target the **Backend API** port or domain (`:5070` by default or your API reverse proxy domain) with path `/signin-oidc`, **not** the web frontend port `:3002`.
:::
...
- **Cause**: The redirect URI in the IdP does not exactly match the backend API's `/signin-oidc` endpoint.
- **Solution**:
  - Verify that the IdP redirect URI is set to `http://<api-host>:5070/signin-oidc` (or `https://api.yourdomain.com/signin-oidc`).
  - Ensure protocol schemes (`http` vs `https`) and ports match.
...
- **Solution**: Set `FRONTEND_URL=http://<server-ip>:3002` (or your reverse proxy public domain) in `docker-compose.yaml` and restart `wiremanager-api`:
  ```bash
